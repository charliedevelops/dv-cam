...
class CaptureManager extends EventEmitter {
  ...
  async startCapture(collectionId: number, collectionName: string): Promise<string> {
    ...
    try {
      // Start the dvrescue process with proper arguments
      ...
      this.processes.set(jobId, dvrescueProcess); // Track process separately
      job.status = "running";
      this.jobs.set(jobId, job);
      ...
      dvrescueProcess.stdout?.on("data", async (data) => {
      ...
      dvrescueProcess.stderr?.on("data", (data) => {
        ...
        console.log(`DVRescue stderr [${jobId}]:`, data.toString());
      });

      // Handle process completion
      dvrescueProcess.on("close", async (code, signal) => {
      ...
      dvrescueProcess.on("error", async (error) => {
        const currentJob = this.jobs.get(jobId);
        if (!currentJob) return;
  ...
  private async parseProgress(jobId: string, output: string): Promise<void> {
    ...
    // Try to parse progress from dvrescue output
